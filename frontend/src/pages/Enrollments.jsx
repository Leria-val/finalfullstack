...
const STATUS_MAP   = { active: "badge badge--approved", inactive: "badge badge--failed" };
...
const Enrollments = () => {
  ...
  const fetchEnrollments = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      showToast("Erro ao carregar matrículas.");
    } finally {
      setLoading(false);
    }
  }, [search, page]);
    ...
    <div className="page-wrapper">
      ...
      <Modal isOpen={modal === "delete"} onClose={() => setModal(null)} title="Cancelar Matrícula" size="sm">
        ...
        <div className="form-row-actions">
          ...
          <button onClick={handleDelete} disabled={saving} className="btn btn--danger">{saving ? "Cancelando..." : "Confirmar"}</button>
        </div>
      </Modal>
    </div>
  );
